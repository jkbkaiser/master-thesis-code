...
class BasicLayer(nn.Module):
    def __init__(
        ...
        if RC_tokens_type == "stem":
            self.RC = PatchEmbedding(
                inter_channel=token_dims // 2,
                out_channels=token_dims,
                img_size=img_size,
            )
        elif downsample_ratios > 1:
            self.RC = ReductionCell(
                img_size,
                in_chans,
                embed_dims,
                token_dims,
...
class ViTAEv2_basic(nn.Module):
    def __init__(
        ...
        self.tokens_dims = (
            token_dims
            if isinstance(token_dims, list)
            else [token_dims * (2**i) for i in range(stages)]
        )
        ...
        for i in range(stages):
            ...
            Layers.append(
                BasicLayer(
                    ...
                    self.dilaions[i],
                    self.RC_op[i],
                    self.RC_tokens_type[i],
                    self.NC_tokens_type[i],
                    self.RC_group[i],
                    self.NC_group[i],
                    self.NC_depth[i],
                    ...
                    qk_scale=self.qk_scale[i],
                    drop=self.drop[i],
                    attn_drop=self.attn_drop[i],
        ...
        # Classifier head
        self.head = (
            nn.Linear(self.tokens_dims[-1], num_classes)
            if num_classes > 0
            else nn.Identity()
        )
