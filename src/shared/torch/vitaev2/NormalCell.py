...
class AttentionPerformer(nn.Module):
    def __init__(
        ...
        for i in range(self.head_cnt):
            self.w[i] = nn.Parameter(
                nn.init.orthogonal_(self.w[i]) * math.sqrt(self.m), requires_grad=False
            )
    ...
    def attn(self, x):
        ...
        kqv = (
            self.kqv(x)
            .reshape(B, N, 3, self.head_cnt, self.head_dim)
            .permute(2, 0, 3, 1, 4)
        )
...
class NormalCell(nn.Module):
    ...
    def forward(self, x):
        ...
        convX = self.drop_path(
            self.PCM(shortcut.view(b, wh, wh, c).permute(0, 3, 1, 2).contiguous())
            .permute(0, 2, 3, 1)
            .contiguous()
            .view(b, n, c)
        )
