...
class PRM(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.share_weights:
            padding = math.ceil(
                ((self.kernel_size - 1) * self.dilations[0] + 1 - self.stride) / 2
            )
            ...
            for i in range(1, len(self.dilations)):
                padding = math.ceil(
                    ((self.kernel_size - 1) * self.dilations[i] + 1 - self.stride) / 2
                )
                _y = nn.functional.conv2d(
                    x,
                    weight=self.convolution.weight,
                    bias=self.convolution.bias,
                    stride=self.downsample_ratio,
        ...
        if self.op == "sum":
            ...
            raise NotImplementedError(
                "no such operation: {} for multi-levels!".format(self.op)
            )
        return y, (W, H)
...
class ReductionCell(nn.Module):
    def __init__(
        ...
        self.PRM = PRM(
            ...
            downsample_ratio=downsample_ratios,
        ...
        if tokens_type == "performer":
            self.attn = Token_performer(
                dim=in_chans, in_dim=token_dims, head_cnt=num_heads, kernel_ratio=0.5
            )
        elif tokens_type == "performer_less":
            self.attn = None
            self.PCM = None
        elif tokens_type == "transformer":
            self.attn = Token_transformer(
                dim=in_chans,
                in_dim=token_dims,
                num_heads=num_heads,
                mlp_ratio=mlp_ratio,
            ...
            self.attn = WindowTransformerBlock(
                in_dim=in_chans,
                out_dim=token_dims,
                input_resolution=(
                    self.img_size // self.downsample_ratios,
                    self.img_size // self.downsample_ratios,
                ),
                num_heads=num_heads,
                mlp_ratio=mlp_ratio,
    ...
    def forward(self, x):
        ...
        if self.tokens_type == "window":
            ...
            if (padding_td + padding_lr) > 0:
                x = x.permute(0, 3, 1, 2)
                x = nn.functional.pad(
                    x, (padding_left, padding_right, padding_top, padding_down)
                )
                x = x.permute(0, 2, 3, 1).contiguous()

            x_windows = window_partition(x, self.window_size)
            x_windows = x_windows.view(-1, self.window_size * self.window_size, C)
            attn_windows = self.attn.attn(
                x_windows, mask=self.attn.attn_mask
            )  # nW*B, window_size*window_size, C
            attn_windows = attn_windows.view(
                -1, self.window_size, self.window_size, self.token_dims
            )
            shifted_x = window_reverse(
                attn_windows, self.window_size, H + padding_td, W + padding_lr
            )  # B H' W' C
            x = shifted_x
